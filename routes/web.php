...
Route::prefix('shipping')->group(function () {
    ...
    // States
    Route::get('/state/view', [ShippingAreaController::class, 'StateView'])->name('manage-state')->middleware('auth:admin');

    Route::get('/district/ajax/{division_id}', [ShippingAreaController::class, 'GetDistrict'])->middleware('auth:admin');

    Route::post('/state/store', [ShippingAreaController::class, 'StateStore'])->name('state.store')->middleware('auth:admin');

    Route::get('/state/edit/{id}', [ShippingAreaController::class, 'StateEdit'])->name('state.edit')->middleware('auth:admin');

    Route::post('/state/update/{id}', [ShippingAreaController::class, 'StateUpdate'])->name('state.update')->middleware('auth:admin');
